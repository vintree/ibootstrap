    ...
    init: function() {
        if(userAgent.isMobile()) {
            ...
            window.addEventListener(_evt, function () {
                //赋值一次
                window.initFont = window.initFont || document.getElementsByTagName('html')[0].getAttribute('style');
                if(userAgent.isMobile()) {
                    clearTimeout(_timer);
                    _timer = setTimeout(setFontSize, 300);
                } else {
                    document.getElementsByTagName('html')[0].setAttribute('style', window.initFont);
                }
            }, false);
            //ios系列
